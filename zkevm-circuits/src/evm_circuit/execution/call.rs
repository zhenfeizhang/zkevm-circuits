...
impl<F: FieldExt> ExecutionGadget<F> for CallGadget<F> {
    ...
    fn configure(cb: &mut ConstraintBuilder<F>) -> Self {
        ...
        let next_memory_word_size =
            cb.copy(memory_expansion.next_memory_word_size());
        // Sum up gas cost
    ...
    fn assign_exec_step(
        ...
        let (next_memory_word_size, memory_expansion_gas_cost) =
        ...
        self.is_account_empty.assign(
            region,
            offset,
            Some(F::from(is_account_empty as u64)),
        )?;
        ...
        self.next_memory_word_size.assign(
            region,
            offset,
            Some(F::from(next_memory_word_size)),
        )?;
        self.one_64th_gas
...
mod test {
    ...
    use bus_mapping::{bytecode, evm::OpcodeId, external_tracer::Account};
