...
impl<F: FieldExt> AbsorbConfig<F> {
    ...
    // 2) NEXT_INPUTS (17 columns) + is_mixing flag (1 column) (offset +1) (current rotation)
    // 3) OUT_STATE (25 columns) (offset +2)
    // TODO: Review with YT!
    pub fn configure(
...
mod tests {
    ...
    fn test_absorb_gate() {
        #[derive(Default)]
        struct MyCircuit<F> {
            // TODO: Review with YT the idea of state being already [(Cell, F;25)] since will always come from copied cells.
            in_state: [F; 25],
            next_input: [F; ABSORB_NEXT_INPUTS],
            is_mixing: bool,
            _marker: PhantomData<F>,
